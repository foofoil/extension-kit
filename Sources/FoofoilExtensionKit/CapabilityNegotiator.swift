...
public struct HostCapabilityContract: Equatable, Sendable {
    public let id: String
    public let maximumContractVersion: UInt32
    public let scope: ExtensionCapabilityScope

    public init(id: String, maximumContractVersion: UInt32, scope: ExtensionCapabilityScope) {
        self.id = id
        self.maximumContractVersion = maximumContractVersion
        self.scope = scope
    }
}

public enum CapabilityRejectionReason: String, Equatable, Sendable {
...
public struct RejectedCapability: Equatable, Sendable {
    public let declaration: ExtensionCapabilityDeclaration
    public let reason: CapabilityRejectionReason

    public init(declaration: ExtensionCapabilityDeclaration, reason: CapabilityRejectionReason) {
        self.declaration = declaration
        self.reason = reason
    }
}

public struct CapabilityNegotiationResult: Equatable, Sendable {
    public let accepted: [NegotiatedCapability]
    public let rejected: [RejectedCapability]

    public init(accepted: [NegotiatedCapability], rejected: [RejectedCapability]) {
...
public enum CapabilityNegotiator {
    public static let v1HostContracts: [HostCapabilityContract] = [
        .init(id: ExtensionCapabilityIdentifier.mediaTransport, maximumContractVersion: 1, scope: .session),
        .init(id: ExtensionCapabilityIdentifier.navigatorActions, maximumContractVersion: 1, scope: .presentation),
